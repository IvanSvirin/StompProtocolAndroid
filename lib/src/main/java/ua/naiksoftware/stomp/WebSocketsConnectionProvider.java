...
class WebSocketsConnectionProvider implements ConnectionProvider {
    ...
    private void createWebSocketConnection() {
        ...
        mWebSocketClient = new WebSocketClient(URI.create(mUri), new Draft_17(), mConnectHttpHeaders, 0) {
            ...
            public void onMessage(String message) {
                ...
                emitLifecycleEvent(new LifecycleEvent(LifecycleEvent.Type.MESSAGE, message));
                emitMessage(message);
            }
    ...
    public Observable<Void> send(StompMessage stompMessage) {
        if (stompMessage.getStompCommand().equals(StompCommand.SEND)) {
            if (mWebSocketClient != null) mWebSocketClient.send(stompMessage.compile());
        }
        return Observable.create(subscriber -> {
            if (mWebSocketClient == null) {
                subscriber.onError(new IllegalStateException("Not connected yet"));
            } else {
                mWebSocketClient.send(stompMessage.compile());
